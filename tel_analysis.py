...
def main(time_str, interval=30, mag=10, nbins_alt=5, nbins_azi=20):
    """selected observable stars & make the script for MakeScript.py
        Args:
            ...
            mag: catalog mag from 9 to 12 magnitude (default: 10)
    ...
    df_tmp, df_ra, df_dec = load_2mass_catalog()
    ...
    for az_bin in range(start_az_bin, nbins_azi):
        for alt_bin in range(start_alt_bin, nbins_alt):
            ...
            if alt_bin == start_alt_bin and az_bin == start_az_bin:
                start_altaz_coords = calc_altaz_coords(df_ra, df_dec, obs_time, obs_loc) # calc alt/azi coords @ obs start time
    ...
    print(f"script for MakeScript.py was saved as \"./output/script_tmp.txt\" ")
    print("--------------------------")
...
if __name__ == "__main__":
    ...
    parser.add_argument("--mag", type=int, default=10, help="catalog magnitude from 9 to 12 (default: 10)")
